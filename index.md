---
layout: default
---

# ✌️Привет, я Ярослав 
[![LinkedIn](https://img.shields.io/badge/LinkedIn-2B7FF0?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/yaroslavkorotkikh) [![Telegram](https://img.shields.io/badge/Telegram-29A7E1?style=flat&logo=telegram&logoColor=white)](https://t.me/yourawsluv) [![Email](https://img.shields.io/badge/Email-D14836?style=flat&logo=gmail&logoColor=white)](mailto:yaroslawkorotkih@yandex.ru)

## Обо мне

>5+ лет в продуктах B2B, SaaS, Fintech. T-shaped экспертиза — от проведения CustDev в MVP, до оптимизации процессов в успешном бизнесе. Работаю на одном языке с UX, A/B, ML, BizDev, Frontend, QA, и конечно же с юзерами:)

---

## Кейсы

### [**CRM**](https://yourawsluv.github.io/CRM.html)
#CRM
#B2B
#Enterprise

[![Figma](https://img.shields.io/badge/Project-Link-blue?style=flat&logo=figma)](https://www.figma.com/design/mPmRLoOHm15smFCwSIVbpX/CRM-CallCenter-Sales?node-id=0-1&t=bLbmOHoYSl3m8Jh4-1)

<img width="600" height="auto" alt="image" src="https://github.com/user-attachments/assets/1e456593-045f-403b-9007-703b9dc7bb1d" />

---

### [**ERP**](https://yourawsluv.github.io/FLORA.html)
#ERP
#B2B
#Enterprise

[![Figma](https://img.shields.io/badge/Project-Link-blue?style=flat&logo=figma)](https://www.figma.com/design/mPmRLoOHm15smFCwSIVbpX/CRM-%D0%9A%D0%A6-%D0%9F%D1%80%D0%BE%D0%B4%D0%B0%D0%B6%D0%B8?node-id=0-1&t=bLbmOHoYSl3m8Jh4-1)

<img width="600" height="auto" alt="image" src="https://github.com/user-attachments/assets/1dc323e0-640a-4843-8f90-5e2496fdfefd" />

---

### [**Платформа для управления цифровыми активами бизнеса**](https://yourawsluv.github.io/Kanoe.html)
#Blockchain
#dApp
#B2B
#Fintech

[![Figma](https://img.shields.io/badge/Project-Link-blue?style=flat&logo=figma)](https://www.figma.com/design/larczXzN5oQlcRHiUXW2LZ/Kanoe-Wallet-SRS-App?node-id=59-40224&t=fQiC5Y1CPyVBDLbx-1)

<img width="600" height="auto" alt="image" src="https://github.com/user-attachments/assets/9a41a2a3-6e24-4765-9f88-5696f0a8706b" />

---

### [**Платформа для управления бизнес-процессами**](https://yourawsluv.github.io/MirrorCFA.html)
#DAO
#dApp
#B2B

[![Figma](https://img.shields.io/badge/Project-Link-blue?style=flat&logo=figma)](https://www.figma.com/design/R9ceAELlyO5wcHKliHjsJi/MirrorDAO?node-id=27-13020&t=ABdDyV4J8sNSSgJn-1)

<img width="600" height="auto" alt="image" src="https://github.com/user-attachments/assets/2668c88d-e31d-486a-9961-0d0a111a57b4" />

---

### [**App for Medical Clinic**](https://yourawsluv.github.io/MedClinicApp.html)
#B2C
#MedTech

[![Figma](https://img.shields.io/badge/Project-Link-blue?style=flat&logo=figma)](https://www.figma.com/design/YbUYgjHSbdNDqSPotkTkY4/Hadassah?node-id=34-5255&t=oPPrG0CGYrswUZ3g-1)

<img width="600" height="auto" alt="image" src="https://github.com/user-attachments/assets/bb16952b-c40b-44ad-acfa-76f3b6fb03d5" />

---

### [**Telegram Mini App (Blockchain)**](https://yourawsluv.github.io/Claimy.html)
#Telegram
#Game
#Blockchain

[![Figma](https://img.shields.io/badge/Project-Link-blue?style=flat&logo=figma)](https://www.figma.com/design/mZq6FqGmvr4eLyLuvayZQ5/Claimy-TON-Tigers?node-id=1666-18109&t=kgjBkEgKKlwvPgl5-1)

<img src="https://github.com/user-attachments/assets/e99889da-7d5b-4e0f-be98-f7184ad075a1" width="600" height="auto">

---

### [**Telegram Mini App (Blockchain)**](https://yourawsluv.github.io/Varbusters.html)
#Telegram
#Game
#Blockchain

[![Figma](https://img.shields.io/badge/Project-Link-blue?style=flat&logo=figma)](https://www.figma.com/design/l2E5DBMhhp6L2z6ksEqjp2/Varbusters-Game-(Copy)?node-id=0-1&t=KfgssFNvIRH9Xib8-1)

<img src="https://github.com/user-attachments/assets/286271a8-3daa-4809-92e9-9d6408708288" width="600" height="auto">

---

### [**Telegram Mini App (Blockchain)**](https://yourawsluv.github.io/NoirRoulette.html)
#Telegram
#Game
#Blockchain

[![Figma](https://img.shields.io/badge/Project-Link-blue?style=flat&logo=figma)](https://www.figma.com/design/l2E5DBMhhp6L2z6ksEqjp2/Varbusters-Game-(Copy)?node-id=0-1&t=KfgssFNvIRH9Xib8-1)

<img src="https://github.com/user-attachments/assets/7a7dfeb2-24c9-4577-999b-9fee473df015" width="600" height="auto">

---

## Опыт работы

### Senior Product Designer
**[DDX](https://www.ddxfitness.ru/) | Moscow, Russia | (2025 — present)**

**B2E продукт CRM/ERP для контактного центра**

- В команде с продактом и аналитиком провел серию интервью с пользователями (отделы Клиентского сервиса и Продаж, Операторы и Руководители) и исследование рыночных решений
- Описал целевые UX-метрики операторов и руководителей воронок (время ответа, время обработки, CSAT)
- Анализ требований для rule-based системы и нагрузки операторов для улучшения опыта конечных клиентов
- Проектировал user-flow и UJM сложной логики бизнес-процессов (Capacity planning, Workforce management), выдвинул продуктовые гипотезы из болей пользователя через фрейморк JTBD
- Спроектировал интерфейс для автоматизации работы колл-центра, KPI дашборда, интеграции с Чат-платформой мобильного приложения
- Ускорил TTD, через рефакторинг дизайн-системы (300+ компонентов) и перенос библиотеки в Storybook для тестового стенда
- Проводил демо перед бизнесом и бенчмарк сценариев на фокус-группе пользователей

### Senior Product Designer
**[ROLF](https://www.rolftech.ru/) | Moscow, Russia | (2024 — 2025)**

**B2E продукт ERP-системы сети дилерских центров**
- провел исследование требований бизнес-логики заказчика и сценариев пользователей совместно с аналитиками
- разработал User Flow нового модуля системы по управлению заказами для оптовой торговли
- спроектировал интерфейс и интерактивный прототип для защиты демо
- поддерживал и расширял единую дизайн-систему платформы
- коммуницировал с командой продукта (PO, BA, SA и Frontend) и приоритезировал бэклог для пилота
- проводил кросс-ревью и планирование с командой продуктовых дизайнеров


### Product Designer
**[NeuroCity™](https://www.neuro-city.ru/) | Voronezh, Russia | (2022 – 2024)**

**B2B продукт NeuroAdmin для управления IaaS сетью устройств навигации и самообслуживания для [Москва-Сити](https://vimeo.com/378351798/b2604eb15c), [ВДНХ](https://vimeo.com/943970322/ec0e09b707), [Шереметьево](https://vimeo.com/844888420/8d661d04fd), [ДЕПО](https://vimeo.com/575738187/691569bb52), [ЦУМ](https://vimeo.com/553351861/36f351f7f5)**
- разрабатывал интерфейс веб-приложения для администрирования, аналитики и мониторинга инфраструктуры заказчика.
- проводил защиты демо перед заказчиками вместе с PM.
- участвовал в продуктовых исследованиях по фреймворку JTBD, составлял CJM для построении гипотез.
- разрабатывал прототипы для пресейла и MVP. Собирал UI-киты проектов. Работал над промышленным дизайном устройств совместно с командой инженеров.
- оформлял бизнес-питчи и участвовал в CustDev совместно с PM и SM для привлечения новых заказчиков.
- создавал и поддерживал дизайн-систему
- проводил ревью, тестировал сборки и координировал работу других дизайнеров.


### Communication Designer
**[Expedition](https://explip.ru/) | Voronezh, Russia | (2021 — 2022)**

- Вёрстка продукции компании для публикации в цифровом и печатном виде.
- Разработка и поддержка дизайна веб-сайта компании.
- Создание единой дизайн-системы в Figma.
- Создание иллюстраций в Photoshop и Illustrator.
- Брендирование и копирайтинг.
- Менторинг младших сотрудников.

### Web Designer/Developer
**Freelance | Voronezh, Russia | (2019 — 2021)**

- Разработка макетов, прототипирование интерфейсов (web, mobile).
- Разработка сайтов с помощью low-code инструментов (Webflow, Tilda).
- Создание презентаций и инфографики.
- Создание цифровой графики (арты, контент, реклама).
- Брендирование и создание документации. Создание UI-kit и библиотеки шаблонов.
- Создание векторной и 3D графики (моделирование, создание текстур, настройка сцен и света, настройка вывода рендера), ретушь фото, анимация(3D, 2D).

---

## Навыки

### Харды
- **Проектирование:** Проектирование (UI), Прототипирование (UX), Визуальный и Коммуникационный дизайн, Дизайн-системы
- **Исследования:** Интервью/опросы, Юзабилити-тестирование, Конкурентный анализ, User Flow, CJM, A/B-тестирование, Empathy Map, Heatmap

### Тулзы
- **Проектирование:** Figma, Sketch, Miro
- **Веб-разработка:** Webflow, Framer, Tilda
- **Frontend Tools:** HTML/CSS, JavaScript, Git, SQL
- **Guidelines:** HIG, MD3
- **Аналитика:** AppsFlyer, UxCam, Amplitude, NPS
- **3D/Motion:** Blender, Spline, Adobe After Effect
- **AI Tools:** Figma Make, GPT, Dall-E, Relume

### Софты
Agile/Scrum, Презентация решения, Работа в команде, Понимание бизнес-логики, Адаптивность, Самообучение, Организация и приоритеты**

<!--
**yourawsluv/yourawsluv** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.
-->
